...
`\<` 或 `\b` : **单词左分界符**
> * 精确匹配到在 "\<" 或 "\b" 之内的单词，表示一个单词左边并没有任何字母，就满足分界符的匹配规则，"\b" 在`字符组`内，表示匹配一个`退格符`。
```javascript
...
`\>` 或 `\b` : **单词右分界符**
> * 精确匹配到在 "\>" 或 "\b" 之内的单词，表示一个单词右边并没有任何字母，就满足分界符的匹配规则，"\b" 在`字符组`内，表示匹配一个`退格符`。
```javascript
    // 由于 正则表达式 流派问题，才会出现两种单词分界符。
    var str = "hello bhello";
...
`\< \>` 或 `\b \b` : **单词左右分界符**
> * 精确匹配一个单词，左右两边都不能存在任何单词，"\b" 在`字符组`内，表示匹配一个`退格符`。
```javascript
